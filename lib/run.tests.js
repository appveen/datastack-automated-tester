...
async function __updateResult(_id, _test, _response) {
	let status = "PASS"
	let comment = null;
	if (_test.responseCode != _response.statusCode) {
		status = "FAIL"
		comment = "Status code mismatch"
	}
	const data = {
		"endDate": new Date(),
...
async function makeAPICall(_token, _test, _resultId) {
	...
	await req(options)
		.then(_response => __updateResult(_resultId, _test, _response))
		.catch(_err => __updateResult(_resultId, _test, _err));
};
	...
	while (true) {
		...
		if (test) {
			...
			try {
				...
				await makeAPICall(tokens[test.method], test, resultId)
